...
const SEQ = "dev::sportid";

function sportsCreate(param) {
...
function sportsUpdate(param) {
  ...
  var uStmt;
  uStmt = param.connection.prepareStatement(`UPDATE \"${USER_TABLE}\" SET \"name\" = '${oUser.name}' WHERE \"sportid\" = ${oUser.sportid};`);
  uStmt.executeUpdate();
  uStmt.close();
}
